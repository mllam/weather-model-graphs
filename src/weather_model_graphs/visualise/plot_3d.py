...
def _build_coastline_trace(radius: float) -> "_go.Scatter3d":
    """Génère la trace des continents pour Plotly."""
    xs, ys, zs = [], [], []
    for lat, lon in zip(COAST_LATS, COAST_LONS):
        if lat is None:
            xs.append(None); ys.append(None); zs.append(None)
        else:
            # On appelle la fonction de conversion qui existe déjà juste au-dessus
...
def render_with_plotly(
    ...
    *,
    ...
    traces = []
    if add_coastlines:
        # Coastlines are added at the top of the figure
        traces.append(_build_coastline_trace(radius=0.99))
