...
2. **Connectivity creation** (``create_directed_mesh_graph`` in ``general.py``)
   -> ``nx.DiGraph`` with ``len`` and ``vdiff`` edge attributes.
...
def create_flat_singlescale_triangular_mesh_graph(
    xy: np.ndarray, mesh_node_distance: float
) -> networkx.DiGraph:
    """
...
def create_flat_multiscale_triangular_mesh_graph(
    ...
    xy : np.ndarray
        Grid point coordinates, shaped ``[N_grid_points, 2]``.
    mesh_node_distance : float
        Distance between mesh nodes at the finest level.
    level_refinement_factor : int
...
def create_hierarchical_triangular_mesh_graph(
    ...
    level_refinement_factor: int,
    max_num_levels: int,
    intra_level: dict = None,
    inter_level: dict = None,
) -> networkx.DiGraph:
    """
    ...
    intra_level : dict, optional
        Keyword arguments for intra-level connectivity (e.g. ``{"pattern": "4-star"}``).
        Defaults to ``{"pattern": "4-star"}``.
    inter_level : dict, optional
        Keyword arguments for inter-level connectivity. If None, uses defaults
        from ``create_hierarchical_from_coordinates``.

    Returns
    -------
    networkx.DiGraph
        Hierarchical triangular mesh graph.
    """
