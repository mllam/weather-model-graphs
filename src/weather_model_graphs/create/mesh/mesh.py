...
def create_single_level_2d_mesh_coordinates(xy, nx, ny):
    """
    Create an undirected mesh graph (nx.Graph) with node positions and spatial
    adjacency edges, representing the coordinate creation step.

    This produces a graph where:
    - Nodes have "pos" (spatial coordinates) and "type" ("mesh") attributes
    - Edges have "adjacency_type" attribute: "cardinal" for horizontal/vertical
      neighbours (4-star) or "diagonal" for diagonal neighbours (8-star only)
    ...
    -------
    networkx.Graph
        Undirected graph with node positions and annotated spatial adjacency edges
    """
...
def create_directed_mesh_graph(G_undirected, pattern="8-star"):
    """
    Convert an undirected mesh graph with spatial adjacency edges to a
    ...
    1. Coordinate creation (create_single_level_2d_mesh_coordinates) -> nx.Graph
    2. Connectivity creation (this function) -> nx.DiGraph

    Parameters
    ----------
    G_undirected : networkx.Graph
        Undirected graph with nodes having "pos" attributes and edges having
        "adjacency_type" attributes ("cardinal" or "diagonal").
    pattern : str
    ...
    -------
    networkx.DiGraph
        Directed graph with bidirectional edges, each having "len" and "vdiff"
        attributes.
    """
    if pattern == "4-star":
...
def create_single_level_2d_mesh_graph(xy, nx, ny):
    ...
    """
    G_coords = create_single_level_2d_mesh_coordinates(xy, nx, ny)
    return create_directed_mesh_graph(G_coords, pattern="8-star")


def create_multirange_2d_mesh_coordinates(
    max_num_levels, xy, grid_spacing=3, interlevel_refinement_factor=3
):
    """
    Create a list of undirected coordinate graphs (nx.Graph) representing
    different levels of mesh resolution spanning the spatial domain of the
    xy coordinates.

    This is the coordinate creation step for multi-level meshes. Each returned
    graph contains nodes with spatial positions and edges annotated with
    adjacency type ("cardinal" or "diagonal").
    ...
    G_all_levels : list of networkx.Graph
        List of undirected coordinate graphs for each level, each with
        node positions and annotated spatial adjacency edges.
        Each graph has "level" and "interlevel_refinement_factor" graph attributes.
    """
    ...
    # in both directions
    max_nodes_bottom = (coord_extent / grid_spacing).astype(int)
    ...
    for lev in range(mesh_levels_to_create):  # 0-index mesh levels
        ...
        g = create_single_level_2d_mesh_coordinates(xy, nodes_x, nodes_y)
