...
def _check_required_graph_attributes(G: networkx.Graph, context: str):
    ...
    ----------
    G : networkx.Graph
        The coordinate graph to validate.
    context : str
        Description of where this check is being called, for error messages.
    ...
    """
    # Check at least one node has required attributes
    if len(G.nodes) > 0:
        sample_node = next(iter(G.nodes))
        if "pos" not in G.nodes[sample_node]:
            raise ValueError(
                f"{context}: coordinate graph nodes must have a 'pos' attribute "
                "(np.ndarray of shape [2,])."
            )
        if "type" not in G.nodes[sample_node]:
            raise ValueError(
                f"{context}: coordinate graph nodes must have a 'type' attribute."
            )
    # Check at least one edge has required attributes
    if len(G.edges) > 0:
        sample_edge = next(iter(G.edges))
        if "adjacency_type" not in G.edges[sample_edge]:
            raise ValueError(
                f"{context}: coordinate graph edges must have an 'adjacency_type' "
                "attribute ('cardinal' or 'diagonal')."
            )
...
def create_flat_multiscale_from_coordinates(
    ...
    """
    # Validate required attributes on first graph
