...
`poseidon-graphs` is a package for creating, visualising and storing graphs used in message-passing graph-based data-driven weather models.
...
If you simply want to install and use `poseidon-graphs` as-is you can install the most recent release directly from pypi with pip

```bash
python -m pip install poseidon-graphs
```
...
optional extra in `poseidon-graphs`:

```bash
python -m pip install poseidon-graphs[pytorch]
```
...
## Developing `poseidon-graphs`

The easiest way to work on developing `poseidon-graphs` is to fork the [main repo](https://github.com/mllam/weather-model-graphs) under your github account, clone this repo locally, install [pdm](https://pdm-project.org/en/latest/), create a venv with pdm and then install `poseidon-graphs` (and all development dependencies):

```
git clone https://github.com/<your-github-username>/poseidon-graphs
cd poseidon-graphs
pdm venv create
...
The best way to understand how to use `poseidon-graphs` is to look at the [documentation](https://mllam.github.io/weather-model-graphs) (which are executable Jupyter notebooks!), to have look at the tests in [tests/](tests/) or simply to read through the source code.
...
The documentation is built using [Jupyter Book](https://jupyterbook.org/intro.html) and can be found at [https://mllam.github.io/weather-model-graphs](https://mllam.github.io/weather-model-graphs). This includes background on graph-based weather models, the design principles of `poseidon-graphs` and how to use it to create your own graph architectures.
